...
class SDK {
  ...
  /**
   * Connect an embedded wallet to a domain.
   *
   * @summary Connect Embedded Wallet
   */
  connect_embedded_wallet(metadata: types.ConnectEmbeddedWalletMetadataParam): Promise<FetchResponse<200, types.ConnectEmbeddedWalletResponse200>> {
    return this.core.fetch('/api/v2/connect-embedded-wallet/{privy_app_id}/{privy_access_token}', 'get', metadata);
  }

  /**
   * Get a embedded wallet user address for a given Privy DID.
   *
   * @summary Get Embedded Wallet Address
   */
  get_embedded_wallet(metadata: types.GetEmbeddedWalletMetadataParam): Promise<FetchResponse<200, types.GetEmbeddedWalletResponse200>> {
    return this.core.fetch('/api/v2/embedded-wallets/{user_id}', 'get', metadata);
  }
  ...
  get_account(metadata: types.GetAccountMetadataParam): Promise<FetchResponse<200, types.GetAccountResponse200>> {
    return this.core.fetch('/api/v2/accounts/{address}', 'get', metadata);
  }
